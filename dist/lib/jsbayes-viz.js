      ...
      (function (window) {
        ...
        function newGraph() {
          return {
            nodes: [],
            edges: [],
            addEdge: function addEdge(parent, child) {
              var edge = {
                parent: parent,
                child: child
              };
            ...
            addNode: function addNode(id, label, values, probs) {
              var width = 150;
        ...
        function drawNodeBars(graph) {
          ...
          for (var i = 0; i < graph.nodes.length; i++) {
            ...
            for (var j = 0; j < node.values.length; j++) {
              ...
              d3.select(selector).text(formatPct(node.probs[j]));
            }
          }
        }
        ...
        function drawNodes(options) {
          ...
          var nodes = d3.select(options.id).selectAll('g').data(graph.nodes).enter().append('g').attr({
            id: function id(d) {
              return d.id;
            },
          ...
          nodes.each(function (d) {
            ...
            for (var i = 0; i < d.values.length; i++) {
              d3.select(this).append('text').attr({
                ...
                g.sample(SAMPLES).then(function (r) {
                  drawNodeBars(graph);
                });
              }).text(function (d) {
        ...
        function drawEdges(options) {
          var graph = options.graph;
          var edges = d3.select(options.id).selectAll('line').data(graph.edges).enter().append('line').each(function (d) {
            var points = graph.edge(d.parent, d.child);
            d3.select(this).attr({
...
//# sourceMappingURL=jsbayes-viz.js.map
